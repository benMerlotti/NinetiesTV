...
{
    return shows
        .Where(s => s.Genres.Contains("Comedy")) // Filter comedy shows
        .OrderByDescending(s => s.ImdbRating)    // Sort by IMDB rating
...
{
    return shows
        .Where(s => s.EndYear >= 2000) // Filter for shows that ended on or after 2000
        .OrderBy(s => s.EndYear)       // Order by the ending year
        .First();                      // Get the first show that satisfies the condition
}
...
{
    return shows
        .OrderByDescending(s => s.Name.Split(' ').Length) // Split by spaces and order by word count
        .First(); // Return the first show, which has the most words
}
...
static string AllNamesWithCommasPlsAnd(List<Show> shows)
{
    return shows
        .Select(s => s.Name)
        .Aggregate((a, b) => a + ", " + b)  // Concatenate all names with commas
        .ReplaceLast(",", " and");  // Replace the last comma with "and"
}
